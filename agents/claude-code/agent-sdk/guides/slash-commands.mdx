...
<Note>
The `.claude/commands/` directory is the legacy format. The recommended format is `.claude/skills/<name>/SKILL.md`, which supports the same slash-command invocation (`/name`) plus autonomous invocation by Claude. See [Skills](/agents/claude-code/agent-sdk/guides/skills) for the current format. The CLI continues to support both formats, and the examples below remain accurate for `.claude/commands/`.
</Note>
...
- [Subagents in the SDK](/agents/claude-code/agent-sdk/guides/subagents) - Similar filesystem-based configuration for subagents
