...
<Steps>
  ...
  <Step title="The SDK collects registered hooks">
    The SDK checks for hooks registered for that event type. This includes callback hooks you pass in `options.hooks` and shell command hooks from settings files, but only if you explicitly load them with [`settingSources`](/agents/claude-code/agent-sdk/sdk-references/typescript#setting-source) or [`setting_sources`](https://docs.claude.com/en/agent-sdk/python#setting-source).
  </Step>
...
|--------|------|---------|-------------|
| `matcher` | `string` | `undefined` | Regex pattern matched against the event's filter field. For tool hooks, this is the tool name. Built-in tools include `Bash`, `Read`, `Write`, `Edit`, `Glob`, `Grep`, `WebFetch`, `Agent`, and others (see [Tool Input Types](/agents/claude-code/agent-sdk/sdk-references/typescript#tool-input-types) for the full list). MCP tools use the pattern `mcp__<server>__<action>`. |
...
<Tip>
**Discovering tool names:** See [Tool Input Types](/agents/claude-code/agent-sdk/sdk-references/typescript#tool-input-types) for the full list of built-in tool names, or add a hook without a matcher to log all tool calls your session makes.
...
Use `SubagentStop` hooks to monitor when subagents finish their work. See the full input type in the [TypeScript](/agents/claude-code/agent-sdk/sdk-references/typescript#hook-input) and [Python](https://docs.claude.com/en/agent-sdk/python#hook-input) SDK references. This example logs a summary each time a subagent completes:
...
- [TypeScript SDK reference](/agents/claude-code/agent-sdk/sdk-references/typescript): hook types, input/output definitions, and configuration options
...
- [Permissions](/agents/claude-code/agent-sdk/guides/permissions): control what your agent can do
- [Custom tools](/agents/claude-code/agent-sdk/guides/custom-tools): build tools to extend agent capabilities
