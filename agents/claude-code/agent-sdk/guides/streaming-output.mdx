...
<Tip>
This page covers output streaming (receiving tokens in real-time). For input modes (how you send messages), see [Send messages to agents](/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode). You can also [stream responses using the Agent SDK via the CLI](https://code.claude.com/docs/en/headless).
</Tip>
...
- **Structured output**: the JSON result appears only in the final `ResultMessage.structured_output`, not as streaming deltas. See [structured outputs](/agents/claude-code/agent-sdk/guides/structured-output) for details.

## Next steps

Now that you can stream text and tool calls in real-time, explore these related topics:

- [Interactive vs one-shot queries](/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode): choose between input modes for your use case
- [Structured outputs](/agents/claude-code/agent-sdk/guides/structured-output): get typed JSON responses from the agent
- [Permissions](/agents/claude-code/agent-sdk/guides/permissions): control which tools the agent can use
