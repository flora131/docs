...
<Note>
**Prefer `allowedTools` over permission modes for MCP access.** `permissionMode: "acceptEdits"` does not auto-approve MCP tools (only file edits and filesystem Bash commands). `permissionMode: "bypassPermissions"` does auto-approve MCP tools but also disables all other safety prompts, which is broader than necessary. A wildcard in `allowedTools` grants exactly the MCP server you want and nothing more. See [Permission modes](/agents/claude-code/agent-sdk/guides/permissions#permission-modes) for a full comparison.
</Note>
...
Define custom tools directly in your application code instead of running a separate server process. See the [custom tools guide](/agents/claude-code/agent-sdk/guides/custom-tools) for implementation details.
...
Tool search is enabled by default. See [Tool search](/agents/claude-code/agent-sdk/guides/tool-search) for configuration options and details.

For more detail, including best practices and using tool search with custom SDK tools, see the [tool search guide](/agents/claude-code/agent-sdk/guides/tool-search).
...
- **[Custom tools guide](/agents/claude-code/agent-sdk/guides/custom-tools)**: Build your own MCP server that runs in-process with your SDK application
- **[Permissions](/agents/claude-code/agent-sdk/guides/permissions)**: Control which MCP tools your agent can use with `allowedTools` and `disallowedTools`
- **[TypeScript SDK reference](/agents/claude-code/agent-sdk/sdk-references/typescript)**: Full API reference including MCP configuration options
