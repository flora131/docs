...
1. **Tool needs approval**: Claude wants to use a tool that isn't auto-approved by [permission rules](/agents/claude-code/agent-sdk/guides/permissions) or modes. Check `tool_name` for the tool (e.g., `"Bash"`, `"Write"`).
...
<Note>
To automatically allow or deny tools without prompting users, use [hooks](/agents/claude-code/agent-sdk/guides/hooks) instead. Hooks execute before `canUseTool` and can allow, deny, or modify requests based on your own logic. You can also use the [`PermissionRequest` hook](/agents/claude-code/agent-sdk/guides/hooks#available-hooks) to send external notifications (Slack, email, push) when Claude is waiting for approval.
</Note>
...
See the SDK reference for complete input schemas: [Python](https://docs.claude.com/en/agent-sdk/python#tool-input-output-types) | [TypeScript](/agents/claude-code/agent-sdk/sdk-references/typescript#tool-input-types).
...
<Note>
In Python, `can_use_tool` requires [streaming mode](/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode) and a `PreToolUse` hook that returns `{"continue_": True}` to keep the stream open. Without this hook, the stream closes before the permission callback can be invoked.
</Note>
...
- **Redirect entirely**: use [streaming input](/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode) to send Claude a completely new instruction
...
<Tabs>
  ...
  <Tab title="Redirect entirely">
    For a complete change of direction (not just a nudge), use [streaming input](/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode) to send Claude a new instruction directly. This bypasses the current tool request and gives Claude entirely new instructions to follow.
  </Tab>
</Tabs>
...
Use [streaming input](/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode) when you need to:
...
Use [custom tools](/agents/claude-code/agent-sdk/guides/custom-tools) when you need to:
...
- [Configure permissions](/agents/claude-code/agent-sdk/guides/permissions): set up permission modes and rules
- [Control execution with hooks](/agents/claude-code/agent-sdk/guides/hooks): run custom code at key points in the agent lifecycle
- [TypeScript SDK reference](/agents/claude-code/agent-sdk/sdk-references/typescript#can-use-tool): full canUseTool API documentation
