...
<Note>
This example uses streaming to show progress in real-time. If you don't need live output (e.g., for background jobs or CI pipelines), you can collect all messages at once. See [Streaming vs. single-turn mode](/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode) for details.
</Note>
...
The example above uses `acceptEdits` mode, which auto-approves file operations so the agent can run without interactive prompts. If you want to prompt users for approval, use `default` mode and provide a [`canUseTool` callback](/agents/claude-code/agent-sdk/guides/user-input) that collects user input. For more control, see [Permissions](/agents/claude-code/agent-sdk/guides/permissions).

## Next steps

Now that you've created your first agent, learn how to extend its capabilities and tailor it to your use case:

- **[Permissions](/agents/claude-code/agent-sdk/guides/permissions)**: control what your agent can do and when it needs approval
- **[Hooks](/agents/claude-code/agent-sdk/guides/hooks)**: run custom code before or after tool calls
- **[Sessions](/agents/claude-code/agent-sdk/core-concepts/sessions)**: build multi-turn agents that maintain context
- **[MCP servers](/agents/claude-code/agent-sdk/guides/mcp)**: connect to databases, browsers, APIs, and other external systems
- **[Hosting](/agents/claude-code/agent-sdk/guides/hosting)**: deploy agents to Docker, cloud, and CI/CD
