---
title: "Copilot SDK for Node.js and TypeScript"
description: "TypeScript SDK for programmatic control of the GitHub Copilot CLI via JSON-RPC."
icon: "code"
---
...
- `gitHubToken?: string` - Per-session GitHub token for authentication. This is the supported token option when connecting through an existing `cliUrl` server.
...
Inbound trace context from the CLI is available on the `ToolInvocation` object passed to tool handlers as `traceparent` and `tracestate` fields. See the [OpenTelemetry guide](https://github.com/github/copilot-cli) for a full wire-up example.
