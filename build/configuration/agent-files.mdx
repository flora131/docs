...
See [context files](/reference/cli/context-files).
...
Setting `scm` in [settings.json](/build/configuration/settings) reconciles GitHub / Azure DevOps MCP servers across each agent's config on every `atomic chat` / `atomic workflow` startup:
...
Override chat flags or env vars per agent in [settings.json](/build/configuration/settings#providers-example):
