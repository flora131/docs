...
description: Reference documentation for terminal setup in Atomic.
---
...
If you want `Shift+Enter` to keep working in tmux via that remap, add `ctrl+j` to your Atomic `newLine` keybinding in `~/.atomic/agent/keybindings.json`:
...
If you want the hardware cursor visible, set `PI_HARDWARE_CURSOR=1` before running Atomic (disabled by default for compatibility).
