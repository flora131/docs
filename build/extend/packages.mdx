...
description: Reference documentation for Atomic packages.
---

> Atomic can help you create Atomic packages. Ask it to bundle your extensions, skills, prompt templates, or themes.


Atomic packages bundle extensions, skills, prompt templates, and themes so you can share them through npm or git. A package declares resources in `package.json` under the `pi` key (the inherited manifest key used by the runtime), or uses conventional directories.
...
If no `pi` manifest is present, Atomic auto-discovers resources from these directories:
...
Atomic bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@bastani/atomic`, `@earendil-works/pi-ai`, `@earendil-works/pi-tui`, `typebox`.
...
Use `atomic config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. Works for both global (`~/.atomic/agent`) and project (`.atomic/`) scopes.
