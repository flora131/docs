---
title: Extensions
description: Reference documentation for extensions in Atomic.
---
...
> **Placement for /reload:** Put extensions in `~/.atomic/agent/extensions/` (global) or `.atomic/extensions/` (project-local) for auto-discovery. Use `atomic -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
To share extensions via npm or git as Atomic packages, see [packages.md](/build/extend/packages).
...
See [Session Format](/reference/runtime/session-format) for session storage internals and the SessionManager API.
...
Fired on compaction. See [compaction.md](/reference/runtime/compaction) for details.
...
Fired on `/tree` navigation. See [Sessions](/reference/runtime/sessions) for tree navigation concepts.
...
Fired before each LLM call. Modify messages non-destructively. See [Session Format](/reference/runtime/session-format) for message types.
...
`false` in print mode (`-p`) and JSON mode. `true` in interactive and RPC mode. In RPC mode, dialog methods (`select`, `confirm`, `input`, `editor`) work via the extension UI sub-protocol, and fire-and-forget methods (`notify`, `setStatus`, `setWidget`, `setTitle`, `setEditorText`) emit requests to the client. Some TUI-specific methods are no-ops or return defaults (see [rpc.md](/reference/runtime/rpc#extension-ui-protocol)).
...
Read-only access to session state. See [Session Format](/reference/runtime/session-format) for the full SessionManager API and entry types.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `atomic` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
- Captured old `atomic` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
Register a keyboard shortcut. See [keybindings.md](/reference/cli/keybindings) for the shortcut format and built-in keybindings.
...
Set the current model. Returns `false` if no API key is available for the model. See [models.md](/build/integrations/custom-models) for configuring custom models.
...
See [custom-provider.md](/build/integrations/custom-providers) for advanced topics: custom streaming APIs, OAuth details, model definition reference.
...
- [read.ts](https://github.com/flora131/atomic/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/flora131/atomic/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/flora131/atomic/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/flora131/atomic/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/flora131/atomic/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/flora131/atomic/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/flora131/atomic/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](/reference/tui/components) for the full component API and [tool-execution.ts](https://github.com/flora131/atomic/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
For the exhaustive list of keybinding ids and defaults, see [keybindings.md](/reference/cli/keybindings). `keybindings.json` uses those same namespaced ids.
...
**For custom components, see [tui.md](/reference/tui/components)** which has copy-paste patterns for:
...
See [tui.md](/reference/tui/components) for the full component API.
...
See [tui.md](/reference/tui/components) for the full `OverlayOptions` API and [overlay-qa-tests.ts](https://github.com/flora131/atomic/blob/main/packages/coding-agent/examples/extensions/overlay-qa-tests.ts) for examples.
...
See [tui.md](/reference/tui/components) Pattern 7 for a complete example with mode indicator.
...
All render functions receive a `theme` object. See [themes.md](/build/extend/themes) for creating custom themes and the full color palette.
...
| RPC (`--mode rpc`) | JSON protocol | Host handles UI, see [rpc.md](/reference/runtime/rpc) |
| JSON (`--mode json`) | No-op | Event stream to stdout, see [json.md](/reference/runtime/json-event-stream) |
