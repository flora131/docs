---
title: Atomic MCP Adapter
description: Reference documentation for Atomic MCP adapter.
---


Use MCP servers from Atomic without burning your context window.
...
```bash
atomic install npm:pi-mcp-adapter
```

Restart Atomic after installation.
...
|---------------------|--------------|
| `.mcp.json` or `~/.config/mcp/mcp.json` | Atomic uses it immediately. The first time you open `/mcp`, you'll see a short heads-up explaining which file Atomic detected and that Atomic only writes adapter-specific overrides to its own files. |
...
If you prefer the terminal, you can also run `pi-mcp-adapter init` after install to scan for host-specific configs and add missing compatibility imports to the Atomic agent dir (`~/.atomic/agent/mcp.json` by default, or `$ATOMIC_DIR/mcp.json` when set).
...
Atomic also reads Pi-owned override files for settings and host-specific compatibility:

- `<Atomic agent dir>/mcp.json` — Atomic global override (`~/.atomic/agent/mcp.json` by default)
- `.atomic/mcp.json` — Atomic project override

Precedence is:

1. `~/.config/mcp/mcp.json`
2. `<Atomic agent dir>/mcp.json`
3. `.mcp.json`
4. `.atomic/mcp.json`
...
| `<Atomic agent dir>/mcp.json` | Atomic global override and compatibility imports (`~/.atomic/agent/mcp.json` by default) |
| `.atomic/mcp.json` | Atomic project override |

Pi-specific files are the write targets for imported or shared global servers when Atomic needs to persist adapter-only settings such as `directTools`.
...
| `sampling` | Allow MCP servers to sample through Atomic models, honoring `modelPreferences.hints` before current/default fallback (default: true when UI approval is available). |
...
| Value | Behavior |
|-------|----------|
| `true` | Register all tools from this server as individual Atomic tools |
...
Direct tools register from the metadata cache in the Atomic agent dir (`~/.atomic/agent/mcp-cache.json` by default, or `$ATOMIC_DIR/mcp-cache.json` when set), so no server connections are needed at startup. On the first session after adding `directTools` to a new server, the cache won't exist yet — tools fall back to proxy-only and the cache populates in the background. To force it: `/mcp reconnect <server>`.
...
3. atomic-mcp-adapter fetches the UI HTML and opens it in an iframe
4. The UI can call MCP tools and send messages back to the agent

**Native rendering:** On macOS, if [Glimpse](https://github.com/hazat/glimpse) is installed (`atomic install npm:glimpseui`), UIs open in a native WKWebView window instead of a browser tab. Set `MCP_UI_VIEWER=browser` to force the browser, or `MCP_UI_VIEWER=glimpse` to require native rendering.
...
Restart Atomic, then ask the agent to show a chart — it calls `show_chart` and opens the UI in Glimpse (macOS) or the browser. Use `npm run uninstall-local` to remove the MCP entry.
...
`pi-mcp-adapter init` detects these host-specific configs and adds missing imports to the Atomic agent dir config for you.

### Project Config

Prefer `.mcp.json` for project-local shared MCP config. Use `.atomic/mcp.json` only when you need a Pi-specific project override. Project files override both user-global shared MCP config and Atomic global overrides.
...
Search includes both MCP tools and Atomic tools (from extensions). Atomic tools appear first with `[pi tool]` prefix. Space-separated words are OR'd.
...
- Specific tools can be promoted from the proxy to first-class Atomic tools via `directTools` config, so the LLM sees them directly instead of having to search

## Limitations

- Cross-session server sharing not yet implemented (each Atomic session runs its own server processes)
