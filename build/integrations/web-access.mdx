---
title: Atomic web access
description: Reference documentation for Atomic web access.
---


**Web search, content extraction, and video understanding for Atomic agent. Zero-config Exa search, optional browser-cookie Gemini Web, or bring your own API keys.**
...
```bash
atomic install npm:pi-web-access
```

Works immediately with no API keys — Exa MCP provides zero-config search. For more providers or direct API access, add keys to `~/.atomic/web-search.json`:
...
Persists to `~/.atomic/web-search.json` and takes effect on the next `web_search` call. When disabled, `web_search` returns raw results without opening the curator window.
...
All config lives in `~/.atomic/web-search.json`. Every field is optional.
...
Both shortcuts are configurable via `~/.atomic/web-search.json`:
...
Values use the same format as Atomic keybindings (e.g. `ctrl+s`, `ctrl+shift+s`, `alt+r`). Changes take effect on next Atomic restart.

Set `"enabled": false` under any feature to disable it. Config changes require a Atomic restart.
