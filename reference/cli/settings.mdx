---
title: Settings
description: Reference documentation for settings in Atomic.
---
...
`enableInstallTelemetry` only controls the anonymous install/update ping to `https://pi.dev/api/report-install`. Opting out of telemetry does not disable update checks; Atomic can still fetch `https://pi.dev/api/latest-version` to look for the latest version.

Set `ATOMIC_SKIP_VERSION_CHECK=1` to disable the Atomic version update check. Use `--offline` or `ATOMIC_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
When multiple sources specify a session directory, precedence is `--session-dir`, `ATOMIC_SESSION_DIR`, then `sessionDir` in settings.json.
...
```json
{
  "packages": ["@bastani/workflows", "@bastani/subagents", "@org/my-extension"]
}
```
...
See [packages.md](/build/extend/packages) for package management details.
