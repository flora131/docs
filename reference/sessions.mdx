...
For the JSONL file format and SessionManager API, see [Session Format](/reference/session-format).
...
| `/compact [prompt]` | Summarize older context; see [Compaction](/reference/compaction) |
...
Filter modes are: default, no-tools, user-only, labeled-only, and all. Configure the default with `treeFilterMode` in [Settings](/reference/settings).
...
See [Compaction](/reference/compaction) for branch summarization internals and extension hooks.
...
For parsers, extensions, SDK usage, and the full SessionManager API, see [Session Format](/reference/session-format).
