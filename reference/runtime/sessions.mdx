---
title: Sessions
description: Reference documentation for sessions in Atomic.
---
...
For the JSONL file format and SessionManager API, see [Session Format](/reference/runtime/session-format).
...
| `/compact [prompt]` | Summarize older context; see [Compaction](/reference/runtime/compaction) |
...
Filter modes are: default, no-tools, user-only, labeled-only, and all. Configure the default with `treeFilterMode` in [Settings](/reference/cli/settings).
...
See [Compaction](/reference/runtime/compaction) for branch summarization internals and extension hooks.
...
For parsers, extensions, SDK usage, and the full SessionManager API, see [Session Format](/reference/runtime/session-format).
