...
description: "Track token usage, deduplicate parallel tool calls, and calculate costs with the Claude Agent SDK."
icon: "coins"
---
...
For complete API documentation, see the [TypeScript SDK reference](/reference/agents/claude-code/agent-sdk/sdk-references/typescript) and [Python SDK reference](https://docs.claude.com/en/agent-sdk/python).
...
- **`query()` call:** one invocation of the SDK's `query()` function. A single call can involve multiple steps (Claude responds, uses tools, gets results, responds again). Each call produces one [`result`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#sdk-result-message) message at the end.
...
The examples in this section use TypeScript field names. In Python, the equivalent fields are [`AssistantMessage.usage`](https://docs.claude.com/en/agent-sdk/python#assistant-message) and `AssistantMessage.message_id` for per-step usage, and [`ResultMessage.model_usage`](https://docs.claude.com/en/agent-sdk/python#result-message) for per-model breakdowns.
...
The Agent SDK automatically uses [prompt caching](https://docs.claude.com/en/build-with-claude/prompt-caching) to reduce costs on repeated content. You do not need to configure caching yourself. The usage object includes two additional fields for cache tracking:
...
Track these separately from `input_tokens` to understand caching savings. In TypeScript, these fields are typed on the [`Usage`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#usage) object. In Python, they appear as keys in the [`ResultMessage.usage`](https://docs.claude.com/en/agent-sdk/python#result-message) dict (for example, `message.usage.get("cache_read_input_tokens", 0)`).

## Related documentation

- [TypeScript SDK Reference](/reference/agents/claude-code/agent-sdk/sdk-references/typescript) - Complete API documentation
- [SDK Overview](/reference/agents/claude-code/agent-sdk/overview) - Getting started with the SDK
- [SDK Permissions](/reference/agents/claude-code/agent-sdk/guides/permissions) - Managing tool permissions
