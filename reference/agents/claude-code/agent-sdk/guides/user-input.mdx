...
description: "Surface Claude's approval requests and clarifying questions to users, then return their decisions to the SDK."
icon: "user-check"
---
...
1. **Tool needs approval**: Claude wants to use a tool that isn't auto-approved by [permission rules](/reference/agents/claude-code/agent-sdk/guides/permissions) or modes. Check `tool_name` for the tool (e.g., `"Bash"`, `"Write"`).
...
<Note>
To automatically allow or deny tools without prompting users, use [hooks](/reference/agents/claude-code/agent-sdk/guides/hooks) instead. Hooks execute before `canUseTool` and can allow, deny, or modify requests based on your own logic. You can also use the [`PermissionRequest` hook](/reference/agents/claude-code/agent-sdk/guides/hooks#available-hooks) to send external notifications (Slack, email, push) when Claude is waiting for approval.
</Note>
...
| `options` (TS) / `context` (Python) | Additional context including optional `suggestions` (proposed `PermissionUpdate` entries to avoid re-prompting) and a cancellation signal. In TypeScript, `signal` is an `AbortSignal`; in Python, the signal field is reserved for future use. See [`ToolPermissionContext`](https://docs.claude.com/en/agent-sdk/python#tool-permission-context) for Python. |
...
See the SDK reference for complete input schemas: [Python](https://docs.claude.com/en/agent-sdk/python#tool-input-output-types) | [TypeScript](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#tool-input-types).
...
<Note>
In Python, `can_use_tool` requires [streaming mode](/reference/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode) and a `PreToolUse` hook that returns `{"continue_": True}` to keep the stream open. Without this hook, the stream closes before the permission callback can be invoked.
</Note>
...
- **Redirect entirely**: use [streaming input](/reference/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode) to send Claude a completely new instruction
...
<Tabs>
  ...
  <Tab title="Redirect entirely">
    For a complete change of direction (not just a nudge), use [streaming input](/reference/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode) to send Claude a new instruction directly. This bypasses the current tool request and gives Claude entirely new instructions to follow.
  </Tab>
</Tabs>
...
<Tip>
Clarifying questions are especially common in [`plan` mode](/reference/agents/claude-code/agent-sdk/guides/permissions#plan-mode-plan), where Claude explores the codebase and asks questions before proposing a plan. This makes plan mode ideal for interactive workflows where you want Claude to gather requirements before making changes.
</Tip>
...
Use [streaming input](/reference/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode) when you need to:
...
Use [custom tools](/reference/agents/claude-code/agent-sdk/guides/custom-tools) when you need to:
...
- [Configure permissions](/reference/agents/claude-code/agent-sdk/guides/permissions): set up permission modes and rules
- [Control execution with hooks](/reference/agents/claude-code/agent-sdk/guides/hooks): run custom code at key points in the agent lifecycle
- [TypeScript SDK reference](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#can-use-tool): full canUseTool API documentation
