...
description: "Intercept and customize agent behavior at key execution points with hooks in the Claude Agent SDK."
...
<Steps>
  ...
  <Step title="The SDK collects registered hooks">
    The SDK checks for hooks registered for that event type. This includes callback hooks you pass in `options.hooks` and shell command hooks from settings files, but only if you explicitly load them with [`settingSources`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#setting-source) or [`setting_sources`](https://docs.claude.com/en/agent-sdk/python#setting-source).
  </Step>
...
|--------|------|---------|-------------|
| `matcher` | `string` | `undefined` | Regex pattern matched against the event's filter field. For tool hooks, this is the tool name. Built-in tools include `Bash`, `Read`, `Write`, `Edit`, `Glob`, `Grep`, `WebFetch`, `Agent`, and others (see [Tool Input Types](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#tool-input-types) for the full list). MCP tools use the pattern `mcp__<server>__<action>`. |
...
<Tip>
**Discovering tool names:** See [Tool Input Types](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#tool-input-types) for the full list of built-in tool names, or add a hook without a matcher to log all tool calls your session makes.
...
Return `{}` to allow the operation without changes. SDK callback hooks use the same JSON output format as [Claude Code shell command hooks](https://code.claude.com/docs/en/hooks#json-output), which documents every field and event-specific option. For the SDK type definitions, see the [TypeScript](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#sync-hook-json-output) and [Python](https://docs.claude.com/en/agent-sdk/python#sync-hook-json-output) SDK references.
...
Use `SubagentStop` hooks to monitor when subagents finish their work. See the full input type in the [TypeScript](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#hook-input) and [Python](https://docs.claude.com/en/agent-sdk/python#hook-input) SDK references. This example logs a summary each time a subagent completes:
...
- Hooks may not fire when the agent hits the [`max_turns`](https://docs.claude.com/en/agent-sdk/python#claude-agent-options) limit because the session ends before hooks can execute
...
- [TypeScript SDK reference](/reference/agents/claude-code/agent-sdk/sdk-references/typescript): hook types, input/output definitions, and configuration options
- [Python SDK reference](https://docs.claude.com/en/agent-sdk/python): hook types, input/output definitions, and configuration options
- [Permissions](/reference/agents/claude-code/agent-sdk/guides/permissions): control what your agent can do
- [Custom tools](/reference/agents/claude-code/agent-sdk/guides/custom-tools): build tools to extend agent capabilities
