...
description: "Get real-time responses from the Agent SDK as text and tool calls stream in."
icon: "stream"
---
...
<Tip>
This page covers output streaming (receiving tokens in real-time). For input modes (how you send messages), see [Send messages to agents](/reference/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode). You can also [stream responses using the Agent SDK via the CLI](https://code.claude.com/docs/en/headless).
</Tip>
...
The `event` field contains the raw streaming event from the [Claude API](https://docs.claude.com/en/build-with-claude/streaming#event-types). Common event types include:
...
- **Structured output**: the JSON result appears only in the final `ResultMessage.structured_output`, not as streaming deltas. See [structured outputs](/reference/agents/claude-code/agent-sdk/guides/structured-output) for details.

## Next steps

Now that you can stream text and tool calls in real-time, explore these related topics:

- [Interactive vs one-shot queries](/reference/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode): choose between input modes for your use case
- [Structured outputs](/reference/agents/claude-code/agent-sdk/guides/structured-output): get typed JSON responses from the agent
- [Permissions](/reference/agents/claude-code/agent-sdk/guides/permissions): control which tools the agent can use
