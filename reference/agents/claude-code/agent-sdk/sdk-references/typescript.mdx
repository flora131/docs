...
description: "Complete API reference for the Claude Agent SDK TypeScript package, including functions, types, and options."
icon: "code"
---
...
<Note>
  **Try the new V2 interface (preview):** A simplified interface with `send()` and `stream()` patterns is now available, making multi-turn conversations easier. [Learn more about the TypeScript V2 preview](https://docs.claude.com/en/agent-sdk/typescript-v2-preview)
</Note>
...
| `allowedTools`                    | `string[]`                                                                                               | `[]`                                        | Tools to auto-approve without prompting. This does not restrict Claude to only these tools; unlisted tools fall through to `permissionMode` and `canUseTool`. Use `disallowedTools` to block tools. See [Permissions](https://docs.claude.com/en/agent-sdk/permissions#allow-and-deny-rules)                                                                                                                                                                                                               |
...
| `enableFileCheckpointing`         | `boolean`                                                                                                | `false`                                     | Enable file change tracking for rewinding. See [File checkpointing](https://docs.claude.com/en/agent-sdk/file-checkpointing)                                                                                                                                                                                                                                                                                                                                                                               |
| `env`                             | `Record<string, string \| undefined>`                                                                    | `process.env`                               | Environment variables. See [Environment variables](https://docs.claude.com/en/env-vars) for variables the underlying CLI reads. Set `CLAUDE_AGENT_SDK_CLIENT_APP` to identify your app in the User-Agent header                                                                                                                                                                                                                                                                                            |
...
| `maxBudgetUsd`                    | `number`                                                                                                 | `undefined`                                 | Stop the query when the client-side cost estimate reaches this USD value. Compared against the same estimate as `total_cost_usd`; see [Track cost and usage](https://docs.claude.com/en/agent-sdk/cost-tracking) for accuracy caveats                                                                                                                                                                                                                                                                      |
...
| `outputFormat`                    | `{ type: 'json_schema', schema: JSONSchema }`                                                            | `undefined`                                 | Define output format for agent results. See [Structured outputs](https://docs.claude.com/en/agent-sdk/structured-outputs) for details                                                                                                                                                                                                                                                                                                                                                                      |
...
| `plugins`                         | [`SdkPluginConfig`](#sdkpluginconfig)`[]`                                                                | `[]`                                        | Load custom plugins from local paths. See [Plugins](https://docs.claude.com/en/agent-sdk/plugins) for details                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `sessionStore`                    | [`SessionStore`](https://docs.claude.com/en/agent-sdk/session-storage#the-sessionstore-interface)                               | `undefined`                                 | Mirror session transcripts to an external backend so any host can resume them. See [Persist sessions to external storage](https://docs.claude.com/en/agent-sdk/session-storage)                                                                                                                                                                                                                                                                                                                            |
| `settingSources`                  | [`SettingSource`](#settingsource)`[]`                                                                    | CLI defaults (all sources)                  | Control which filesystem settings to load. Pass `[]` to disable user, project, and local settings. Managed policy settings load regardless. See [Use Claude Code features](https://docs.claude.com/en/agent-sdk/claude-code-features#what-settingsources-does-not-control)                                                                                                                                                                                                                                 |
...
| `rewindFiles(userMessageId, options?)` | Restores files to their state at the specified user message. Pass `{ dryRun: true }` to preview changes. Requires `enableFileCheckpointing: true`. See [File checkpointing](https://docs.claude.com/en/agent-sdk/file-checkpointing) |
...
| :------------------------------ | :--------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `askUserQuestion.previewFormat` | `'markdown' \| 'html'` | Opts into the `preview` field on [`AskUserQuestion`](https://docs.claude.com/en/agent-sdk/user-input#question-format) options and sets its content format. When unset, Claude does not emit previews |
...
For complete information on creating and using plugins, see [Plugins](https://docs.claude.com/en/agent-sdk/plugins).
...
Plugin installation progress event. Emitted when [`CLAUDE_CODE_SYNC_PLUGIN_INSTALL`](https://docs.claude.com/en/env-vars) is set, so your Agent SDK application can track marketplace plugin installation before the first turn. The `started` and `completed` statuses bracket the overall install. The `installed` and `failed` statuses report individual marketplaces and include `name`.
...
| `channel`           | Message arriving on a [channel](https://docs.claude.com/en/channels). `server` is the source MCP server name.                                                  |
...
| `coordinator`       | Message from a team coordinator in an [agent team](https://docs.claude.com/en/agent-teams).                                                                    |

## Hook Types

For a comprehensive guide on using hooks with examples and common patterns, see the [Hooks guide](https://docs.claude.com/en/agent-sdk/hooks).
...
Asks the user clarifying questions during execution. See [Handle approvals and user input](https://docs.claude.com/en/agent-sdk/user-input#handle-clarifying-questions) for usage details.
...
Per-model usage statistics returned in result messages. The `costUSD` value is a client-side estimate. See [Track cost and usage](https://docs.claude.com/en/agent-sdk/cost-tracking) for billing caveats.
...
* [SDK overview](https://docs.claude.com/en/agent-sdk/overview) - General SDK concepts
* [Python SDK reference](https://docs.claude.com/en/agent-sdk/python) - Python SDK documentation
* [CLI reference](https://docs.claude.com/en/cli-reference) - Command-line interface
* [Common workflows](https://docs.claude.com/en/common-workflows) - Step-by-step guides
