...
![Agent loop: prompt enters, Claude evaluates, branches to tool calls or final answer](https://docs.claude.com/docs/images/agent-loop-diagram.svg)
...
3. **Execute tools.** The SDK runs each requested tool and collects the results. Each set of tool results feeds back to Claude for the next decision. You can use [hooks](/reference/agents/claude-code/agent-sdk/guides/hooks) to intercept, modify, or block tool calls before they run.
...
- **`SystemMessage`:** session lifecycle events. The `subtype` field distinguishes them: `"init"` is the first message (session metadata), and `"compact_boundary"` fires after [compaction](#automatic-compaction). In TypeScript, the compact boundary is its own [`SDKCompactBoundaryMessage`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#sdk-compact-boundary-message) type rather than a subtype of `SDKSystemMessage`.
...
- **`StreamEvent`:** only emitted when partial messages are enabled. Contains raw API streaming events (text deltas, tool input chunks). See [Stream responses](/reference/agents/claude-code/agent-sdk/guides/streaming-output).
...
- **Live streaming:** enable partial messages (`include_partial_messages` in Python, `includePartialMessages` in TypeScript) to get `StreamEvent` messages in real time. See [Stream responses in real-time](/reference/agents/claude-code/agent-sdk/guides/streaming-output).
...
- **Connect external services** with [MCP servers](/reference/agents/claude-code/agent-sdk/guides/mcp) (databases, browsers, APIs)
- **Define custom tools** with [custom tool handlers](/reference/agents/claude-code/agent-sdk/guides/custom-tools)
- **Load project skills** via [setting sources](/reference/agents/claude-code/agent-sdk/core-concepts/claude-code-features) for reusable workflows
...
- **`disallowed_tools` / `disallowedTools`** blocks listed tools, regardless of other settings. See [Permissions](/reference/agents/claude-code/agent-sdk/guides/permissions) for the order that rules are checked before a tool runs.
...
You can also scope individual tools with rules like `"Bash(npm:*)"` to allow only specific commands. See [Permissions](/reference/agents/claude-code/agent-sdk/guides/permissions) for the full rule syntax.
...
Custom tools default to sequential execution. To enable parallel execution for a custom tool, mark it as read-only in its annotations: `readOnly` in [TypeScript](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#tool) or `readOnlyHint` in [Python](https://docs.claude.com/en/agent-sdk/python#tool).

## Control how the loop runs

You can limit how many turns the loop takes, how much it costs, how deeply Claude reasons, and whether tools require approval before running. All of these are fields on [`ClaudeAgentOptions`](https://docs.claude.com/en/agent-sdk/python#claude-agent-options) (Python) / [`Options`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#options) (TypeScript).
...
When either limit is hit, the SDK returns a `ResultMessage` with a corresponding error subtype (`error_max_turns` or `error_max_budget_usd`). See [Handle the result](#handle-the-result) for how to check these subtypes and [`ClaudeAgentOptions`](https://docs.claude.com/en/agent-sdk/python#claude-agent-options) / [`Options`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#options) for syntax.

### Effort level

The `effort` option controls how much reasoning Claude applies. Lower effort levels use fewer tokens per turn and reduce cost. Not all models support the effort parameter. See [Effort](https://docs.claude.com/en/build-with-claude/effort) for which models support it.
...
For interactive applications, use `"default"` with a tool approval callback to surface approval prompts. For autonomous agents on a dev machine, `"acceptEdits"` auto-approves file edits while still gating `Bash` behind allow rules. Reserve `"bypassPermissions"` for CI, containers, or other isolated environments. See [Permissions](/reference/agents/claude-code/agent-sdk/guides/permissions) for full details.

### Model

If you don't set `model`, the SDK uses Claude Code's default, which depends on your authentication method and subscription. Set it explicitly (for example, `model="claude-sonnet-4-6"`) to pin a specific model or to use a smaller model for faster, cheaper agents. See [models](https://docs.claude.com/en/about-claude/models) for available IDs.

## The context window

The context window is the total amount of information available to Claude during a session. It does not reset between turns within a session. Everything accumulates: the system prompt, tool definitions, conversation history, tool inputs, and tool outputs. Content that stays the same across turns (system prompt, tool definitions, CLAUDE.md) is automatically [prompt cached](https://docs.claude.com/en/build-with-claude/prompt-caching), which reduces cost and latency for repeated prefixes.
...
| **CLAUDE.md files** | Session start, when [`settingSources`](/reference/agents/claude-code/agent-sdk/core-concepts/claude-code-features) is enabled | Full content in every request (but prompt-cached, so only the first request pays full cost) |
| **Tool definitions** | Every request | Each tool adds its schema; use [MCP tool search](/reference/agents/claude-code/agent-sdk/guides/mcp#mcp-tool-search) to load tools on-demand instead of all at once |
...
Compaction replaces older messages with a summary, so specific instructions from early in the conversation may not be preserved. Persistent rules belong in CLAUDE.md (loaded via [`settingSources`](/reference/agents/claude-code/agent-sdk/core-concepts/claude-code-features)) rather than in the initial prompt, because CLAUDE.md content is re-injected on every request.
...
- **`PreCompact` hook:** Run custom logic before compaction occurs, for example to archive the full transcript. The hook receives a `trigger` field (`manual` or `auto`). See [hooks](/reference/agents/claude-code/agent-sdk/guides/hooks).
- **Manual compaction:** Send `/compact` as a prompt string to trigger compaction on demand. (Slash commands sent this way are SDK inputs, not CLI-only shortcuts. See [slash commands in the SDK](/reference/agents/claude-code/agent-sdk/guides/slash-commands).)
...
See [Session management](/reference/agents/claude-code/agent-sdk/core-concepts/sessions) for the full guide on resume, continue, and fork patterns.

<Note>
In Python, `ClaudeSDKClient` handles session IDs automatically across multiple calls. See the [Python SDK reference](https://docs.claude.com/en/agent-sdk/python#choosing-between-query-and-claude-sdk-client) for details.
</Note>
...
The `result` field (the final text output) is only present on the `success` variant, so always check the subtype before reading it. All result subtypes carry `total_cost_usd`, `usage`, `num_turns`, and `session_id` so you can track cost and resume even after errors. In Python, `total_cost_usd` and `usage` are typed as optional and may be `None` on some error paths, so guard before formatting them. See [Tracking costs and usage](/reference/agents/claude-code/agent-sdk/guides/cost-tracking) for details on interpreting the `usage` fields.

The result also includes a `stop_reason` field (`string | null` in TypeScript, `str | None` in Python) indicating why the model stopped generating on its final turn. Common values are `end_turn` (model finished normally), `max_tokens` (hit the output token limit), and `refusal` (the model declined the request). On error result subtypes, `stop_reason` carries the value from the last assistant response before the loop ended. To detect refusals, check `stop_reason === "refusal"` (TypeScript) or `stop_reason == "refusal"` (Python). See [`SDKResultMessage`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#sdk-result-message) (TypeScript) or [`ResultMessage`](https://docs.claude.com/en/agent-sdk/python#result-message) (Python) for the full type.

## Hooks

[Hooks](/reference/agents/claude-code/agent-sdk/guides/hooks) are callbacks that fire at specific points in the loop: before a tool runs, after it returns, when the agent finishes, and so on. Some commonly used hooks are:
...
Both SDKs support all the events above. The TypeScript SDK includes additional events that Python does not yet support. See [Control execution with hooks](/reference/agents/claude-code/agent-sdk/guides/hooks) for the complete event list, per-SDK availability, and the full callback API.
...
- **Haven't run an agent yet?** Start with the [quickstart](/reference/agents/claude-code/agent-sdk/quickstart) to get the SDK installed and see a full example running end to end.
- **Ready to hook into your project?** [Load CLAUDE.md, skills, and filesystem hooks](/reference/agents/claude-code/agent-sdk/core-concepts/claude-code-features) so the agent follows your project conventions automatically.
- **Building an interactive UI?** Enable [streaming](/reference/agents/claude-code/agent-sdk/guides/streaming-output) to show live text and tool calls as the loop runs.
- **Need tighter control over what the agent can do?** Lock down tool access with [permissions](/reference/agents/claude-code/agent-sdk/guides/permissions), and use [hooks](/reference/agents/claude-code/agent-sdk/guides/hooks) to audit, block, or transform tool calls before they execute.
- **Running long or expensive tasks?** Offload isolated work to [subagents](/reference/agents/claude-code/agent-sdk/guides/subagents) to keep your main context lean.
