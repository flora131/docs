...
description: "How sessions persist agent conversation history, and when to use continue, resume, and fork to return to a prior run."
...
<Note>
Sessions persist the **conversation**, not the filesystem. To snapshot and revert file changes the agent made, use [file checkpointing](/reference/agents/claude-code/agent-sdk/guides/file-checkpointing).
</Note>
...
How much session handling you need depends on your application's shape. Session management comes into play when you send multiple prompts that should share context. Within a single `query()` call, the agent already takes as many turns as it needs, and permission prompts and `AskUserQuestion` are [handled in-loop](/reference/agents/claude-code/agent-sdk/guides/user-input) (they don't end the call).
...
| Stateless task, don't want anything written to disk (TypeScript only) | Set [`persistSession: false`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#options). The session exists only in memory for the duration of the call. Python always persists to disk. |

### Continue, resume, and fork

Continue, resume, and fork are option fields you set on `query()` ([`ClaudeAgentOptions`](https://docs.claude.com/en/agent-sdk/python#claude-agent-options) in Python, [`Options`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#options) in TypeScript).
...
[`ClaudeSDKClient`](https://docs.claude.com/en/agent-sdk/python#claude-sdk-client) handles session IDs internally. Each call to `client.query()` automatically continues the same session. Call [`client.receive_response()`](https://docs.claude.com/en/agent-sdk/python#claude-sdk-client) to iterate over the messages for the current query. The client must be used as an async context manager.
...
See the [Python SDK reference](https://docs.claude.com/en/agent-sdk/python#choosing-between-query-and-claude-sdk-client) for details on when to use `ClaudeSDKClient` vs the standalone `query()` function.
...
Resume and fork require a session ID. Read it from the `session_id` field on the result message ([`ResultMessage`](https://docs.claude.com/en/agent-sdk/python#result-message) in Python, [`SDKResultMessage`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#sdk-result-message) in TypeScript), which is present on every result regardless of success or error. In TypeScript the ID is also available earlier as a direct field on the init `SystemMessage`; in Python it's nested inside `SystemMessage.data`.
...
- **Recover from a limit.** The first run ended with `error_max_turns` or `error_max_budget_usd` (see [Handle the result](/reference/agents/claude-code/agent-sdk/agent-loop#handle-the-result)); resume with a higher limit.
...
<Note>
Forking branches the conversation history, not the filesystem. If a forked agent edits files, those changes are real and visible to any session working in the same directory. To branch and revert file changes, use [file checkpointing](/reference/agents/claude-code/agent-sdk/guides/file-checkpointing).
</Note>
...
Both SDKs expose functions for enumerating sessions on disk and reading their messages: [`listSessions()`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#list-sessions) and [`getSessionMessages()`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#get-session-messages) in TypeScript, [`list_sessions()`](https://docs.claude.com/en/agent-sdk/python#list-sessions) and [`get_session_messages()`](https://docs.claude.com/en/agent-sdk/python#get-session-messages) in Python. Use them to build custom session pickers, cleanup logic, or transcript viewers.

Both SDKs also expose functions for looking up and mutating individual sessions: [`get_session_info()`](https://docs.claude.com/en/agent-sdk/python#get-session-info), [`rename_session()`](https://docs.claude.com/en/agent-sdk/python#rename-session), and [`tag_session()`](https://docs.claude.com/en/agent-sdk/python#tag-session) in Python, and [`getSessionInfo()`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#get-session-info), [`renameSession()`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#rename-session), and [`tagSession()`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#tag-session) in TypeScript. Use them to organize sessions by tag or give them human-readable titles.

## Related resources

- [How the agent loop works](/reference/agents/claude-code/agent-sdk/agent-loop): Understand turns, messages, and context accumulation within a session
- [File checkpointing](/reference/agents/claude-code/agent-sdk/guides/file-checkpointing): Track and revert file changes across sessions
- [Python `ClaudeAgentOptions`](https://docs.claude.com/en/agent-sdk/python#claude-agent-options): Full session option reference for Python
- [TypeScript `Options`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#options): Full session option reference for TypeScript
