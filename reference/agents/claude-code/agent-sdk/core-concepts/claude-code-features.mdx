...
description: "Load project instructions, skills, hooks, and other Claude Code features into your SDK agents."
icon: "puzzle-piece"
---
...
The setting sources option ([`setting_sources`](https://docs.claude.com/en/agent-sdk/python#claude-agent-options) in Python, [`settingSources`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#setting-source) in TypeScript) controls which filesystem-based settings the SDK loads. Without it, your agent won't discover skills, `CLAUDE.md` files, or project-level hooks.
...
Each source loads settings from a specific location, where `<cwd>` is the working directory you pass via the `cwd` option (or the process's current directory if unset). For the full type definition, see [`SettingSource`](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#setting-source) (TypeScript) or [`SettingSource`](https://docs.claude.com/en/agent-sdk/python#setting-source) (Python).
...
<Tip>
You can also inject context directly via `systemPrompt` without using CLAUDE.md files. See [Modify system prompts](/reference/agents/claude-code/agent-sdk/guides/modifying-system-prompts). Use CLAUDE.md when you want the same context shared between interactive Claude Code sessions and your SDK agents.
</Tip>
...
<Note>
Skills must be created as filesystem artifacts (`.claude/skills/<name>/SKILL.md`). The SDK does not have a programmatic API for registering skills. See [Agent Skills in the SDK](/reference/agents/claude-code/agent-sdk/guides/skills) for full details.
</Note>

For more on creating and using skills, see [Agent Skills in the SDK](/reference/agents/claude-code/agent-sdk/guides/skills).
...
- **Programmatic hooks:** callback functions passed directly to `query()`. These run in your application process and can return structured decisions. See [Control execution with hooks](/reference/agents/claude-code/agent-sdk/guides/hooks).
...
Hook callbacks receive the tool input and return a decision dict. Returning `{}` (an empty dict) means allow the tool to proceed. Returning `{"decision": "block", "reason": "..."}` prevents execution and the reason is sent to Claude as the tool result. See the [hooks guide](/reference/agents/claude-code/agent-sdk/guides/hooks) for the full callback signature and return types.
...
<Note>
The TypeScript SDK supports additional hook events beyond Python, including `SessionStart`, `SessionEnd`, `TeammateIdle`, and `TaskCompleted`. See the [hooks guide](/reference/agents/claude-code/agent-sdk/guides/hooks) for the full event compatibility table.
</Note>

For full details on programmatic hooks, see [Control execution with hooks](/reference/agents/claude-code/agent-sdk/guides/hooks). For filesystem hook syntax, see [Hooks](https://code.claude.com/docs/en/hooks).
...
| Give the agent reference material it loads when relevant | [Skills](/reference/agents/claude-code/agent-sdk/guides/skills) | `settingSources` + `allowedTools: ["Skill"]` |
| Run a reusable workflow (deploy, review, release) | [User-invocable skills](/reference/agents/claude-code/agent-sdk/guides/skills) | `settingSources` + `allowedTools: ["Skill"]` |
| Delegate an isolated subtask to a fresh context (research, review) | [Subagents](/reference/agents/claude-code/agent-sdk/guides/subagents) | `agents` parameter + `allowedTools: ["Agent"]` |
...
| Run deterministic logic on tool calls (audit, block, transform) | [Hooks](/reference/agents/claude-code/agent-sdk/guides/hooks) | `hooks` parameter with callbacks, or shell scripts loaded via `settingSources` |
| Give Claude structured tool access to an external service | [MCP](/reference/agents/claude-code/agent-sdk/guides/mcp) | `mcpServers` parameter |

<Tip>
**Subagents versus agent teams:** Subagents are ephemeral and isolated: fresh conversation, one task, summary returned to parent. Agent teams coordinate multiple independent Claude Code instances that share a task list and message each other directly. Agent teams are a CLI feature. See [What subagents inherit](/reference/agents/claude-code/agent-sdk/guides/subagents#what-subagents-inherit) and the [agent teams comparison](https://code.claude.com/docs/en/agent-teams#compare-with-subagents) for details.
</Tip>
...
- [Skills in the SDK](/reference/agents/claude-code/agent-sdk/guides/skills): Full guide to using skills programmatically
- [Subagents](/reference/agents/claude-code/agent-sdk/guides/subagents): Define and invoke subagents for isolated subtasks
- [Hooks](/reference/agents/claude-code/agent-sdk/guides/hooks): Intercept and control agent behavior at key execution points
- [Permissions](/reference/agents/claude-code/agent-sdk/guides/permissions): Control tool access with modes, rules, and callbacks
- [System prompts](/reference/agents/claude-code/agent-sdk/guides/modifying-system-prompts): Inject context without CLAUDE.md files
