...
1. **`query`**: the main entry point that creates the agentic loop. It returns an async iterator, so you use `async for` to stream messages as Claude works. See the full API in the [Python](https://docs.claude.com/en/agent-sdk/python#query) or [TypeScript](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#query) SDK reference.

2. **`prompt`**: what you want Claude to do. Claude figures out which tools to use based on the task.

3. **`options`**: configuration for the agent. This example uses `allowedTools` to pre-approve `Read`, `Edit`, and `Glob`, and `permissionMode: "acceptEdits"` to auto-approve file changes. Other options include `systemPrompt`, `mcpServers`, and more. See all options for [Python](https://docs.claude.com/en/agent-sdk/python#claude-agent-options) or [TypeScript](/reference/agents/claude-code/agent-sdk/sdk-references/typescript#options).
...
<Note>
This example uses streaming to show progress in real-time. If you don't need live output (e.g., for background jobs or CI pipelines), you can collect all messages at once. See [Streaming vs. single-turn mode](/reference/agents/claude-code/agent-sdk/guides/streaming-vs-single-mode) for details.
</Note>
...
The example above uses `acceptEdits` mode, which auto-approves file operations so the agent can run without interactive prompts. If you want to prompt users for approval, use `default` mode and provide a [`canUseTool` callback](/reference/agents/claude-code/agent-sdk/guides/user-input) that collects user input. For more control, see [Permissions](/reference/agents/claude-code/agent-sdk/guides/permissions).

## Next steps

Now that you've created your first agent, learn how to extend its capabilities and tailor it to your use case:

- **[Permissions](/reference/agents/claude-code/agent-sdk/guides/permissions)**: control what your agent can do and when it needs approval
- **[Hooks](/reference/agents/claude-code/agent-sdk/guides/hooks)**: run custom code before or after tool calls
- **[Sessions](/reference/agents/claude-code/agent-sdk/core-concepts/sessions)**: build multi-turn agents that maintain context
- **[MCP servers](/reference/agents/claude-code/agent-sdk/guides/mcp)**: connect to databases, browsers, APIs, and other external systems
- **[Hosting](/reference/agents/claude-code/agent-sdk/guides/hosting)**: deploy agents to Docker, cloud, and CI/CD
