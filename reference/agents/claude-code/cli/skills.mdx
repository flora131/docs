---
title: "Agent Skills"
description: "Package expertise into discoverable Skills that Claude Code loads on demand to perform specialized tasks."
icon: "wand-magic-sparkles"
---

Claude Code combines a model that reasons about your code with [built-in tools](https://docs.claude.com/en/how-claude-code-works#tools) for file operations, search, execution, and web access. The built-in tools cover most coding tasks. This guide covers the extension layer: features you add to customize what Claude knows, connect it to external services, and automate workflows.

<Note>
  For how the core agentic loop works, see [How Claude Code works](https://docs.claude.com/en/how-claude-code-works).
</Note>

**New to Claude Code?** Start with [CLAUDE.md](https://docs.claude.com/en/memory) for project conventions. Add other extensions as you need them.

## Overview

Extensions plug into different parts of the agentic loop:

* **[CLAUDE.md](https://docs.claude.com/en/memory)** adds persistent context Claude sees every session
* **[Skills](https://docs.claude.com/en/skills)** add reusable knowledge and invocable workflows
* **[MCP](https://docs.claude.com/en/mcp)** connects Claude to external services and tools
* **[Subagents](https://docs.claude.com/en/sub-agents)** run their own loops in isolated context, returning summaries
* **[Agent teams](https://docs.claude.com/en/agent-teams)** coordinate multiple independent sessions with shared tasks and peer-to-peer messaging
* **[Hooks](https://docs.claude.com/en/hooks)** run outside the loop entirely as deterministic scripts
* **[Plugins](https://docs.claude.com/en/plugins)** and **[marketplaces](https://docs.claude.com/en/plugin-marketplaces)** package and distribute these features

[Skills](https://docs.claude.com/en/skills) are the most flexible extension. A skill is a markdown file containing knowledge, workflows, or instructions. You can invoke skills with a command like `/deploy`, or Claude can load them automatically when relevant. Skills can run in your current conversation or in an isolated context via subagents.
...
| **[Agent teams](https://docs.claude.com/en/agent-teams)** | Coordinate multiple independent Claude Code sessions       | Parallel research, new feature development, debugging with competing hypotheses | Spawn reviewers to check security, performance, and tests simultaneously        |
...
**[Plugins](https://docs.claude.com/en/plugins)** are the packaging layer. A plugin bundles skills, hooks, subagents, and MCP servers into a single installable unit. Plugin skills are namespaced (like `/my-plugin:review`) so multiple plugins can coexist. Use plugins when you want to reuse the same setup across multiple repositories or distribute to others via a **[marketplace](https://docs.claude.com/en/plugin-marketplaces)**.
...
<Tabs>
  <Tab title="Skill vs Subagent">
    ...
    **They can combine.** A subagent can preload specific skills (`skills:` field). A skill can run in isolated context using `context: fork`. See [Skills](https://docs.claude.com/en/skills) for details.
  </Tab>
  ...
  <Tab title="CLAUDE.md vs Skill">
    ...
    **Rule of thumb:** Keep CLAUDE.md under 200 lines. If it's growing, move reference content to skills or split into [`.claude/rules/`](https://docs.claude.com/en/memory#organize-rules-with-claude/rules/) files.
  </Tab>
  ...
  <Tab title="CLAUDE.md vs Rules vs Skills">
    ...
    **Use rules** to keep CLAUDE.md focused. Rules with [`paths` frontmatter](https://docs.claude.com/en/memory#path-specific-rules) only load when Claude works with matching files, saving context.
  ...
  <Tab title="Subagent vs Agent team">
    ...
    **Transition point:** If you're running parallel subagents but hitting context limits, or if your subagents need to communicate with each other, agent teams are the natural next step.
    ...
    <Note>
      Agent teams are experimental and disabled by default. See [agent teams](https://docs.claude.com/en/agent-teams) for setup and current limitations.
    </Note>
  </Tab>
...
* **CLAUDE.md files** are additive: all levels contribute content to Claude's context simultaneously. Files from your working directory and above load at launch; subdirectories load as you work in them. When instructions conflict, Claude uses judgment to reconcile them, with more specific instructions typically taking precedence. See [how CLAUDE.md files load](https://docs.claude.com/en/memory#how-claude-md-files-load).
* **Skills and subagents** override by name: when the same name exists at multiple levels, one definition wins based on priority (managed > user > project for skills; managed > CLI flag > project > user > plugin for subagents). Plugin skills are [namespaced](https://docs.claude.com/en/plugins#add-skills-to-your-plugin) to avoid conflicts. See [skill discovery](https://docs.claude.com/en/skills#where-skills-live) and [subagent scope](https://docs.claude.com/en/sub-agents#choose-the-subagent-scope).
* **MCP servers** override by name: local > project > user. See [MCP scope](https://docs.claude.com/en/mcp#scope-hierarchy-and-precedence).
* **Hooks** merge: all registered hooks fire for their matching events regardless of source. See [hooks](https://docs.claude.com/en/hooks).
...
Every feature you add consumes some of Claude's context. Too much can fill up your context window, but it can also add noise that makes Claude less effective; skills may not trigger correctly, or Claude may lose track of your conventions. Understanding these trade-offs helps you build an effective setup. For an interactive view of how these features combine in a running session, see [Explore the context window](https://docs.claude.com/en/context-window).
...
<Tabs>
  <Tab title="CLAUDE.md">
    ...
    **Inheritance:** Claude reads CLAUDE.md files from your working directory up to the root, and discovers nested ones in subdirectories as it accesses those files. See [How CLAUDE.md files load](https://docs.claude.com/en/memory#how-claude-md-files-load) for details.

    <Tip>Keep CLAUDE.md under \~500 lines. Move reference material to skills, which load on-demand.</Tip>
  </Tab>

  <Tab title="Skills">
    Skills are extra capabilities in Claude's toolkit. They can be reference material (like an API style guide) or invocable workflows you trigger with `/<name>` (like `/deploy`). Claude Code ships with [bundled skills](https://docs.claude.com/en/skills#bundled-skills) like `/simplify`, `/batch`, and `/debug` that work out of the box. You can also create your own. Claude uses skills when appropriate, or you can invoke one directly.
  ...
  <Tab title="MCP servers">
    ...
    **Context cost:** [Tool search](https://docs.claude.com/en/mcp#scale-with-mcp-tool-search) is on by default, so idle MCP tools consume minimal context.
  ...
  <Tab title="Hooks">
    **When:** On trigger. Hooks fire at specific lifecycle events like tool execution, session boundaries, prompt submission, permission requests, and compaction. See [Hooks](https://docs.claude.com/en/hooks) for the full list.
...
<CardGroup cols={2}>
  <Card title="CLAUDE.md" icon="file-lines" href="https://docs.claude.com/en/memory">
    Store project context, conventions, and instructions
  </Card>

  <Card title="Skills" icon="brain" href="https://docs.claude.com/en/skills">
    Give Claude domain expertise and reusable workflows
  </Card>

  <Card title="Subagents" icon="users" href="https://docs.claude.com/en/sub-agents">
    Offload work to isolated context
  </Card>

  <Card title="Agent teams" icon="network" href="https://docs.claude.com/en/agent-teams">
    Coordinate multiple sessions working in parallel
  </Card>

  <Card title="MCP" icon="plug" href="https://docs.claude.com/en/mcp">
    Connect Claude to external services
  </Card>

  <Card title="Hooks" icon="bolt" href="https://docs.claude.com/en/hooks-guide">
    Automate workflows with hooks
  </Card>

  <Card title="Plugins" icon="puzzle-piece" href="https://docs.claude.com/en/plugins">
    Bundle and share feature sets
  </Card>

  <Card title="Marketplaces" icon="store" href="https://docs.claude.com/en/plugin-marketplaces">
