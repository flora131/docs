---
title: "Permissions"
description: "How Claude Code's permission system controls tool access, including allow/deny/ask rules and permission modes."
icon: "shield-halved"
---
...
Claude Code supports several permission modes that control how tools are approved. See [Permission modes](https://docs.claude.com/en/permission-modes) for when to use each one. Set the `defaultMode` in your [settings files](https://docs.claude.com/en/settings#settings-files):
...
To prevent `bypassPermissions` or `auto` mode from being used, set `permissions.disableBypassPermissionsMode` or `disableAutoMode` to `"disable"` in any [settings file](https://docs.claude.com/en/settings#settings-files). These are most useful in [managed settings](#managed-settings) where they cannot be overridden.
...
<Warning>
  Read and Edit deny rules apply to Claude's built-in file tools, not to Bash subprocesses. A `Read(./.env)` deny rule blocks the Read tool but does not prevent `cat .env` in Bash. For OS-level enforcement that blocks all processes from accessing a path, [enable the sandbox](https://docs.claude.com/en/sandboxing).
</Warning>
...
Use `Agent(AgentName)` rules to control which [subagents](https://docs.claude.com/en/sub-agents) Claude can use:
...
A blocking hook also takes precedence over allow rules. A hook that exits with code 2 stops the tool call before permission rules are evaluated, so the block applies even when an allow rule would otherwise let the call proceed. To run all Bash commands without prompts except for a few you want blocked, add `"Bash"` to your allow list and register a PreToolUse hook that rejects those specific commands. See [Block edits to protected files](https://docs.claude.com/en/hooks-guide#block-edits-to-protected-files) for a hook script you can adapt.
...
* **Persistent configuration**: add to `additionalDirectories` in [settings files](https://docs.claude.com/en/settings#settings-files)

Files in additional directories follow the same permission rules as the original working directory: they become readable without prompts, and file editing permissions follow the current permission mode.

## How permissions interact with sandboxing

Permissions and [sandboxing](https://docs.claude.com/en/sandboxing) are complementary security layers:
...
| `allowManagedMcpServersOnly`                   | When `true`, only `allowedMcpServers` from managed settings are respected. `deniedMcpServers` still merges from all sources. See [Managed MCP configuration](https://docs.claude.com/en/mcp#managed-mcp-configuration)                                             |
| `allowedChannelPlugins`                        | Allowlist of channel plugins that may push messages. Replaces the default Anthropic allowlist when set. Requires `channelsEnabled: true`. See [Restrict which channel plugins can run](https://docs.claude.com/en/channels#restrict-which-channel-plugins-can-run) |
| `blockedMarketplaces`                          | Blocklist of marketplace sources. Blocked sources are checked before downloading, so they never touch the filesystem. See [managed marketplace restrictions](https://docs.claude.com/en/plugin-marketplaces#managed-marketplace-restrictions)                      |
...
| `strictKnownMarketplaces`                      | Controls which plugin marketplaces users can add. See [managed marketplace restrictions](https://docs.claude.com/en/plugin-marketplaces#managed-marketplace-restrictions)                                                                                          |

<Note>
  Access to [Remote Control](https://docs.claude.com/en/remote-control) and [web sessions](https://docs.claude.com/en/claude-code-on-the-web) is not controlled by a managed settings key. On Team and Enterprise plans, an admin enables or disables these features in [Claude Code admin settings](https://claude.ai/admin-settings/claude-code).
</Note>
...
Permission rules follow the same [settings precedence](https://docs.claude.com/en/settings#settings-precedence) as all other Claude Code settings:
...
* [Settings](https://docs.claude.com/en/settings): complete configuration reference including the permission settings table
* [Sandboxing](https://docs.claude.com/en/sandboxing): OS-level filesystem and network isolation for Bash commands
* [Authentication](https://docs.claude.com/en/authentication): set up user access to Claude Code
* [Security](https://docs.claude.com/en/security): security safeguards and best practices
* [Hooks](https://docs.claude.com/en/hooks-guide): automate workflows and extend permission evaluation
