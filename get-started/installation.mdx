...
<Note>
The CLI needs a terminal multiplexer (tmux on macOS/Linux, psmux on Windows) and at least one authenticated coding agent CLI at runtime. The multiplexer is auto-installed on the first non-info `atomic` command; the agent CLIs are not. See [Quickstart prerequisites](/get-started/quickstart#prerequisites).
</Note>
