...
| **Read next** | [CLI commands](/reference/cli/commands) | [SDK reference](/sdk/overview) |
...
A common setup: install the CLI for `atomic chat` and the built-in workflows, and `bun add @bastani/atomic-sdk` in your repo to author team-specific workflows. Register those local workflows with the CLI (see [registering workflows](/build/workflows/registering)) so `atomic workflow` discovers them alongside the built-ins.
