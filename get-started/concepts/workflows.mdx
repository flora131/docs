...
- **Retry / loop** — a bounded `for` loop with early exit when a review comes back clean (see the [`review-fix-loop`](/build/workflows/built-ins) pattern).
...
Every stage writes its messages, transcript, and metadata to disk under `~/.atomic/sessions/<runId>/` — a complete, inspectable execution record. See [reading run artifacts](/build/workflows/running#reading-run-artifacts).

<Columns cols={2}>
  <Card title="Build a workflow" icon="hammer" href="/build/workflows/building">
