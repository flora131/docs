...
To share extensions via npm or git as pi packages, see [packages.md](/customization/packages).
...
See [Session Format](/reference/session-format) for session storage internals and the SessionManager API.
...
Fired on compaction. See [compaction.md](/reference/compaction) for details.
...
Fired on `/tree` navigation. See [Sessions](/reference/sessions) for tree navigation concepts.
...
Fired before each LLM call. Modify messages non-destructively. See [Session Format](/reference/session-format) for message types.
...
`false` in print mode (`-p`) and JSON mode. `true` in interactive and RPC mode. In RPC mode, dialog methods (`select`, `confirm`, `input`, `editor`) work via the extension UI sub-protocol, and fire-and-forget methods (`notify`, `setStatus`, `setWidget`, `setTitle`, `setEditorText`) emit requests to the client. Some TUI-specific methods are no-ops or return defaults (see [rpc.md](/reference/rpc#extension-ui-protocol)).
...
Read-only access to session state. See [Session Format](/reference/session-format) for the full SessionManager API and entry types.
...
Register a keyboard shortcut. See [keybindings.md](/reference/keybindings) for the shortcut format and built-in keybindings.
...
Set the current model. Returns `false` if no API key is available for the model. See [models.md](/customization/custom-models) for configuring custom models.
...
See [custom-provider.md](/customization/custom-providers) for advanced topics: custom streaming APIs, OAuth details, model definition reference.
...
For the exhaustive list of keybinding ids and defaults, see [keybindings.md](/reference/keybindings). `keybindings.json` uses those same namespaced ids.
...
All render functions receive a `theme` object. See [themes.md](/customization/themes) for creating custom themes and the full color palette.
...
| RPC (`--mode rpc`) | JSON protocol | Host handles UI, see [rpc.md](/reference/rpc) |
| JSON (`--mode json`) | No-op | Event stream to stdout, see [json.md](/reference/json-event-stream) |
