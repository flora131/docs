...
<Warning>
**Atomic workflows run coding agents with all permission checks disabled.** The agent can read, write, and delete files, execute arbitrary shell commands, and make network requests without prompting. This is required for unattended pipelines. **Run workflows in a [devcontainer](/installation#devcontainer) or a [git worktree](https://git-scm.com/docs/git-worktree), not on your host machine.**
</Warning>
...
Defaults can be overridden per-project via `providers` in `.atomic/settings.json` — `chatFlags` replaces defaults entirely; `envVars` are merged. See [settings](/configuration/settings).
...
Each feature bundles Atomic, Bun, playwright-cli, agent configs, and the agent CLI itself. See [installation](/installation#devcontainer).

### Git worktree

For autonomous coding workflows like [Ralph](/workflows/built-in#ralph), a [git worktree](https://git-scm.com/docs/git-worktree) keeps the agent's changes off your working tree:
