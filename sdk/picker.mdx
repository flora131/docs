...
When you register custom workflows via [settings.json](/build/configuration/settings) and one fails to load, the picker still renders it as a broken row. Pressing Enter on a broken row flashes the reason on the statusline instead of launching the workflow. See [registering workflows](/build/workflows/registering#broken-entries).
