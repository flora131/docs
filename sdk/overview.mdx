...
The SDK runs on **Bun**, not Node.js. You also need a multiplexer (tmux on macOS/Linux, psmux on Windows) and at least one authenticated agent CLI (`claude`, `opencode`, or `copilot`). See [installation](/get-started/installation#sdk-only).
