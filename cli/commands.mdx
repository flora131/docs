...
See [running workflows](/workflows/running) for input precedence, attaching, and reading run artifacts.
...
See [sessions](/concepts/sessions) for naming conventions and detach semantics.
...
The directory contains `messages.json`, `inbox.md`, and `status.json` — see [running workflows](/workflows/running#reading-run-artifacts).

## `atomic workflow refresh`

Reload custom workflows from `settings.json` and report loaded + broken entries with field-by-field diagnostics. Auto-defaults to JSON inside an `atomic chat` session. See [registering workflows](/workflows/registering).
...
`scm` reconciles the GitHub / Azure DevOps MCP servers in agent configs on startup — see [agent files](/configuration/agent-files).
...
For npm installs, the command prints the matching `<pm> update -g` hint instead of running it. See [installation](/installation#updating).
...
See [installation](/installation#uninstalling) for full removal steps.
